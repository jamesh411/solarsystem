...
class Planet {
  ...
  Planet(String n, int nm, float g, float pd, float or, float op, long od) {
    name = n;
    planetNumber = nm;
    gravity = g;
    planetDiameter = log(pd);
    orbitalRadius = log(or/10E6)*50;
    orbitalRadiusRaw = or;
    orbitalPeriod = op;
    orbitalDistance = od;
...
//  }
  ...
  void hover(Planet e) {
    ...
    text("0" + str(planetNumber) + ".", 137, 788);
    fill(255, opacity);
    text(name, 254, 788);
    textSize(16);
    ...
    if (name == "earth") {
      ...
      //float distanceEarth1 = dist(convert(x), convert(y), convert(e.x), convert(e.y));
      float distanceEarth = dist(cos(angle) * orbitalRadiusRaw, sin(angle) * orbitalRadiusRaw, cos(e.angle) * e.orbitalRadiusRaw, sin(e.angle) * e.orbitalRadiusRaw);
  ...
  void decorate() {
    ...
    float scaleVal = 5.0;
    float inc = PI*gravity/90;
    float angleWave = 0.0;
    for (int a = 105; a <= 335; a += 5 ) {
      float b = (sin(angleWave+radians(wave)) * scaleVal);
      ellipse(a, b+825, 10, 15);
      b = (cos(angleWave+PI/4+radians(wave)) * scaleVal);
      ellipse(a, b+825, 10, 15);
      angleWave += inc;
    }
    wave += log(angularVelocity)*2;
  }
  
  float convert(float n) {
    return exp((n-400)/50)*10E6;
